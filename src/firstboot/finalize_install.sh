...
# ── Tuiles OSM hors-ligne (si Ethernet activé et GPS configuré) ──────────────
if [ "$ENABLE_ETHERNET" = "true" ]; then
    FETCH_SCRIPT="/usr/local/bin/sos-guide-fetch-tiles.sh"
    if [ -f "$FETCH_SCRIPT" ] && [ -x "$FETCH_SCRIPT" ]; then
        LAT_CHK=$(jq -r '.establishment.lat // ""' "$CONFIG_FILE")
        if [ -n "$LAT_CHK" ] && [ "$LAT_CHK" != "null" ]; then
            echo "  → Téléchargement tuiles OSM en arrière-plan…"
            nohup bash "$FETCH_SCRIPT" "$CONFIG_FILE" >> /var/log/sos-guide-fetch-tiles.log 2>&1 &
            echo "  ✔ Fetch-tiles lancé (PID $!)"
        else
            echo "  ⚠ GPS non configuré — tuiles OSM non téléchargées"
        fi
    else
        echo "  ⚠ sos-guide-fetch-tiles.sh absent — tuiles OSM ignorées"
    fi
fi
