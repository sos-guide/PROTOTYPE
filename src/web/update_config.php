...
$ssidChanged = ($oldName !== $newName) || $channelChanged;
...
if (is_executable(REGEN_SCRIPT)) {
    ...
    // Fallback inline aligné avec sos-guide-regen-hash.sh
    $parts = [
        'find /var/www/sos-guide -type f ! -name "config.json" ! -name "*.tmp.*" ! -name "*.log"'
            . ' -exec sha256sum {} \; > /root/integrity.hash.tmp',
        '[ -f /var/www/sos-guide/data/config.json ]'
            . ' && sha256sum /var/www/sos-guide/data/config.json >> /root/integrity.hash.tmp',
    ];
    foreach ([
        ...
        '/usr/local/bin/sos-guide-regen-hash.sh',
    ] as $cfg) {
        $parts[] = '[ -f ' . escapeshellarg($cfg) . ' ]'
                 . ' && sha256sum ' . escapeshellarg($cfg) . ' >> /root/integrity.hash.tmp || true';
    }
    $parts[] = 'mv /root/integrity.hash.tmp /root/integrity.hash'
             . ' && chmod 400 /root/integrity.hash';
    $cmd = implode(' && ', $parts);
