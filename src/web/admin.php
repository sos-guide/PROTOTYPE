...
// ── Hash intégrité ────────────────────────────────────────────────────────────
$hashAgeMin  = file_exists(HASH_FILE) ? (int) round((time() - filemtime(HASH_FILE)) / 60) : -1;
$hashAge     = $hashAgeMin >= 0 ? $hashAgeMin . ' min' : 'absent';
$hashStale   = $hashAgeMin < 0 || $hashAgeMin > 10;
...
<div class="top-bar">
  <h1>⛑️ SOS-GUIDE Administration
    <span class="badge badge-ok">v2.5</span>
    <?php if ($degradedMode): ?>
  ...
  <main class="main" id="adminMain">
    ...
    <div class="grid4">
      ...
      <div class="stat-card">
        ...
        <div class="val" style="font-size:1rem;color:var(--<?= $hashStale ? ($hashAgeMin < 0 ? 'red' : 'yellow') : 'green' ?>)">
          <?= $hashAgeMin < 0 ? '✗' : ($hashStale ? '⚠' : '✓') ?>
        </div>
        <div class="lbl" style="color:var(--<?= $hashStale ? ($hashAgeMin < 0 ? 'red' : 'yellow') : 'sub' ?>)">
          Màj il y a <?= $hashAge ?><?= ($hashStale && $hashAgeMin >= 0) ? ' — obsolète' : '' ?>
        </div>
      </div>
      <div class="stat-card">
    ...
    <div class="card" id="services" style="margin-top:1.25rem">
      ...
      <div class="svc-row">
        ...
        <span class="svc-action" style="color:var(--sub)">
          <?= $degradedMode ? '⚠️ Alerte active' : ('màj il y a ' . $hashAge . ($hashStale && $hashAgeMin >= 0 ? ' ⚠' : '')) ?>
        </span>
      </div>
      <?php if (!empty($ethIp)): ?>
