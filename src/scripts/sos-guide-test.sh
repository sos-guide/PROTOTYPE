...
echo ""
echo -e "  ${BOLD}⛑️  SOS-GUIDE v2.5 — Tests d'intégration${NC}"
...
ALL_LANGS=("ar" "cs" "da" "de" "el" "en" "es" "fa" "fi" "fr" "he"
           "hi" "hu" "it" "ja" "ko" "nl" "no" "pl" "pt" "rm" "ro"
...
# ═══════════════════════════════════════════════════════════════════════════════
# TEST 9 — PCi-CH (Protection civile suisse — certification obligatoire)
# ═══════════════════════════════════════════════════════════════════════════════
section "Certification PCi-CH (Protection civile)"

# 9.1 rm.json — présence et validité JSON
RM_JSON="/var/www/sos-guide/data/rm.json"
if [ -f "$RM_JSON" ]; then
    if jq empty "$RM_JSON" 2>/dev/null; then
        pass "rm.json (Romanche) présent et JSON valide — PCi-CH §4.2"
    else
        fail "rm.json invalide" "rm.json n'est pas un JSON valide — PCi-CH bloquant"
    fi
else
    fail "rm.json absent" "$RM_JSON introuvable — certification PCi-CH impossible"
fi

# 9.2 rm.json — structure minimale (clés obligatoires)
if [ -f "$RM_JSON" ] && jq empty "$RM_JSON" 2>/dev/null; then
    for key in "app" "urgences" "sections"; do
        if jq -e ".${key}" "$RM_JSON" &>/dev/null; then
            pass "rm.json clé '${key}' présente"
        else
            warn_t "rm.json structure" "Clé '${key}' absente — vérifier le contenu romanche"
        fi
    done
fi

# 9.3 Flux STARTER → PRODUCTION : binaires présents
if [ -f "/usr/local/bin/sos-guide-install.sh" ] && \
   [ -x "/usr/local/bin/sos-guide-install.sh" ]; then
    pass "sos-guide-install.sh présent et exécutable (source de vérité unique)"
else
    fail "sos-guide-install.sh" "/usr/local/bin/sos-guide-install.sh absent ou non exécutable"
fi

if [ -f "/usr/local/bin/finalize_install.sh" ] && \
   [ -x "/usr/local/bin/finalize_install.sh" ]; then
    # Vérifier que finalize_install.sh est bien un thin wrapper (pas une copie complète)
    LINE_COUNT=$(wc -l < /usr/local/bin/finalize_install.sh 2>/dev/null || echo "999")
    if [ "$LINE_COUNT" -lt 50 ]; then
        pass "finalize_install.sh : thin wrapper ($LINE_COUNT lignes)"
    else
        warn_t "finalize_install.sh" "Fichier volumineux ($LINE_COUNT lignes) — risque de divergence avec sos-guide-install.sh"
    fi
else
    fail "finalize_install.sh" "/usr/local/bin/finalize_install.sh absent"
fi

# 9.4 Sudoers : www-data → finalize_install.sh (appelé par api_install.php)
if grep -q "finalize_install.sh" /etc/sudoers.d/sos-guide-firstboot 2>/dev/null; then
    pass "Sudoers firstboot : www-data → finalize_install.sh"
elif [ ! -f "/var/lib/sos-guide/installed" ]; then
    warn_t "Sudoers firstboot" "Règle absente ET Pi non installé — STARTER bloqué"
else
    pass "Sudoers firstboot : non requis (Pi en mode PRODUCTION)"
fi

# 9.5 Marqueur PRODUCTION : cohérence de l'état
INSTALL_MARKER_F="/var/lib/sos-guide/installed"
if [ -f "$INSTALL_MARKER_F" ]; then
    VER=$(grep "^version=" "$INSTALL_MARKER_F" 2>/dev/null | cut -d= -f2)
    NODE=$(grep "^node=" "$INSTALL_MARKER_F" 2>/dev/null | cut -d= -f2)
    if [ -n "$VER" ] && [ -n "$NODE" ]; then
        pass "État PRODUCTION confirmé — v${VER} · nœud: ${NODE}"
    else
        warn_t "Marqueur installation" "Champs version/node manquants dans $INSTALL_MARKER_F"
    fi
else
    fail "Mode PRODUCTION" "$INSTALL_MARKER_F absent — Pi non finalisé"
fi

# 9.6 Route /api/reload-network-proxy présente dans nginx (bouton Recharger admin)
if grep -q "reload-network-proxy" /etc/nginx/sites-available/sos-guide 2>/dev/null; then
    pass "Route /api/reload-network-proxy dans nginx"
else
    fail "nginx reload-proxy" "Location /api/reload-network-proxy absent — bouton Recharger silencieusement cassé"
fi

# 9.7 ap_isolate=1 confirmé (redondant avec TEST 3 — explicite pour PCi-CH)
if grep -q "^ap_isolate=1" /etc/hostapd/hostapd.conf 2>/dev/null; then
    pass "ap_isolate=1 : clients WiFi isolés entre eux — conformité PCi-CH"
else
    fail "ap_isolate" "ap_isolate=1 absent de hostapd.conf — non-conformité PCi-CH critique"
fi

# 9.8 Quatre langues nationales CH en PRODUCTION
for lang in fr de it rm; do
    f="/var/www/sos-guide/data/${lang}.json"
    if [ -f "$f" ] && jq empty "$f" 2>/dev/null; then
        pass "Langue nationale CH déployée : $lang"
    else
        fail "Langue CH" "${lang}.json absent ou invalide en production — PCi-CH bloquant"
    fi
done
