#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  SOS-GUIDE — sos-guide-boot-check.sh v2.5                                  ║
...
if [ -z "$WIFI_IFACE" ]; then
    ...
    # I-04 : vérifier ap_isolate=1 dans hostapd (clients WiFi ne peuvent pas se voir)
    HOSTAPD_CONF="/etc/hostapd/hostapd.conf"
    if [ -f "$HOSTAPD_CONF" ]; then
        if ! grep -q "^ap_isolate=1" "$HOSTAPD_CONF"; then
            log_event "WARN" "AP_ISOLATE_MISSING" \
                "ap_isolate=1 absent de $HOSTAPD_CONF — clients WiFi peuvent communiquer entre eux"
        else
            log_event "INFO" "AP_ISOLATE_OK" "ap_isolate=1 confirmé dans hostapd"
        fi
    fi
fi
...
# PHP-FPM : redémarrer seulement en mode normal (pas en mode dégradé)
if [ ! -f "$ALERT_FLAG" ]; then
    PHP_VERSION=$(php -v 2>/dev/null | head -n1 | cut -d' ' -f2 | cut -d'.' -f1-2)
    PHP_SVC="php${PHP_VERSION}-fpm"
    if ! systemctl is-active --quiet "$PHP_SVC" 2>/dev/null; then
        log_event "WARN" "PHPFPM_DOWN" "$PHP_SVC arrêté en mode normal — tentative de redémarrage"
        systemctl reset-failed "$PHP_SVC" 2>/dev/null || true
        systemctl start "$PHP_SVC" 2>/dev/null || \
            log_event "ERROR" "PHPFPM_START_FAILED" "$PHP_SVC impossible à démarrer"
    fi
fi
...
log_event "INFO" "BOOT_CHECK_OK" \
    "Vérification v2.5 terminée — $(date -Iseconds)"
